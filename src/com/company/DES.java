...
public class DES {
            ...
            //analogicznie do initialTable. Stosowane w deszyfrowaniu
            int[] inverseInitialTable = { 40, 8, 48, 16, 56, 24, 64,
                    32, 39, 7, 47, 15, 55,
                    23, 63, 31, 38, 6, 46,
                    14, 54, 22, 62, 30, 37,
                    5, 45, 13, 53, 21, 61,
            ...
            //pierwszy klucz 56-bitowy
            int[] firstKeyTable = { 57, 49, 41, 33, 25,
                    ...
                    20, 12, 4 };
            //drugi klucz 48-bitowy
            int[] secondKeyTable = { 14, 17, 11, 24, 1, 5, 3,
                    28, 15, 6, 21, 10, 23, 19, 12,
                    4, 26, 8, 16, 7, 27, 20, 13, 2,
                    41, 52, 31, 37, 47, 55, 30, 40,
                    51, 45, 33, 48, 44, 49, 39, 56,
                    34, 53, 46, 42, 50, 36, 29, 32 };
            //rozszerzona permutacja
            int[] expansionTable = { 32, 1, 2, 3, 4, 5, 4,
                    5, 6, 7, 8, 9, 8, 9, 10,
                    ...
                    29, 30, 31, 32, 1 };
            //permutacja
            int[] permutationTable = { 16, 7, 20, 21, 29, 12, 28,
                    17, 1, 15, 23, 26, 5, 18,
                    31, 10, 2, 8, 24, 14, 32,
                    27, 3, 9, 19, 13, 30, 6,
                    22, 11, 4, 25 };
            ...
            //funkcja relizujaca przesuniecie bitow
            ...
            String[] getKeys(String key)
            {
                ...
                key = permutation(firstKeyTable, key); //permutacja klucza 56 bitowego
                for (int i = 0; i < 16; i++) {
                    //przesuniecie bitow
                    ...
                    keys[i] = permutation(secondKeyTable, key); //permutacja dla nowego 48 bitowego klucza
                }
            ...
            String sBox(String input)
            {
                ...
                for (int i = 0; i < 48; i += 6) {//podzial na 6-bitowe podciagi
                    String temp = input.substring(i, i + 6);
                    int num = i / 6;
                    int row = Integer.parseInt(
                            temp.charAt(0) + "" + temp.charAt(5), 2); //zmiana stringa na int w systemie dwojkowym
                    int col = Integer.parseInt(
                            temp.substring(1, 5), 2);//zmiana stringa na int w systemie dwojkowym
                    output += Integer.toHexString(
                            sbox[num][row][col]); //zwracanie outputa typu string w systemie 16stkowym
                }
                return output;
            }
            ...
            String round(String input, String key, int num)
            {
                ...
                temp = xor(temp, key); // operacji xor z  bitami klucza rundy,
                temp = sBox(temp); //wprowadzenie do S-bloku
                temp = permutation(permutationTable, temp); //permutacja tego
                left = xor(left, temp); //polaczenie czesci prawej i lewej oraz realizacja finalnej permutacji
                return right + left;
            }
}
